...
+ (CBLDatabase *)preference {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        CBLManager *manager = [CBLManager sharedInstance];
        NSError *error;
        CBLDatabase *db = [manager databaseNamed:@"preference" error:&error];
        if (error) {
...
+ (UserPreference *)info {
    CBLQuery *query = [[self preference] createAllDocumentsQuery];
    NSError *error;
    ...
    return [[UserPreference alloc] initWithDictionary:[results rowAtIndex:0].document.properties];
}
...
+ (void)setInfo:(UserPreference *)info {
    ...
    if (error) {
        NSLog(@"SetPreferenceInfo Fail");
        return;
    }
    
    if (results.count == 0) {
        CBLDocument *document = [[self preference] createDocument];
